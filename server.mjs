...
const builtInFinancialEnginePrompt = [
  ...
  "Think like an asymmetric risk engine, not a headline summarizer: classify each idea into risk-return quadrants such as high-risk/high-return, low-risk/moderate-return, high-risk/low-return trap, and no-edge/no-trade.",
  "Do not rely on static heuristics such as RSI-high means short or news-positive means long. Use probability-weighted causal reasoning: evidence -> mechanism -> price/flow reaction -> asymmetry -> invalidation.",
  "Surface non-consensus and tail-risk ideas when evidence supports them, but label them as speculative thesis bets with smaller sizing, tighter invalidation, and explicit leverage caps.",
  "For every ranked setup, state expected upside, defined downside, subjective probability/fit, reward:risk, wick/liquidation room, and why any high-risk/low-return setup should be skipped.",
  ...
  "SETUPS must be conditional: asset | variation | bias | trigger | TP | SL/invalidation | safer leverage band | high-risk band | probability/fit.",
...
async function generateDigest(schedule, trigger = "scheduled") {
  ...
  const message = [
    ...
    "Include a risk-return quadrant line before SETUPS: high-risk/high-return, low-risk/moderate-return, high-risk/low-return trap, or no-edge/no-trade. Avoid heuristic-only calls; justify asymmetry with probability, reward:risk, wick room, and invalidation.",
...
async function answerNowCommand({ chatId, query, schedule }) {
  ...
  const result = await runChat({
    ...
    message: [
      ...
      "SETUPS must contain exactly three execution variations for the strongest setup: Conservative, Base, Aggressive. Each line includes asset, bias, trigger, TP, SL/invalidation, leverage band, probability/fit, and skip condition.",
...
function buildDigestPrompt(record) {
  ...
  return [
    ...
    "Asymmetric risk engine: classify setups by risk-return quadrant, reject high-risk/low-return traps, and prefer probability-weighted asymmetry over static heuristics. Non-consensus/tail bets are allowed only with clear evidence, smaller sizing, hard invalidation, and leverage caps.",
    ...
    `Output: top opportunities, exactly three primary probability scenarios summing to 100%, optional unweighted tail paths, leverage bands, three execution variations (conservative/base/aggressive) with probability/fit, TP, SL/invalidation, AI bottleneck implications, and source ledger.`,
